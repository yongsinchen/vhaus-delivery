    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center z-50 pt-10 px-4 pb-10 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          ...
          <button onClick={() => { setShowVehicleModal(false); setShowAddVehicle(false); setEditVehicleId(null); }} className="text-gray-400 hover:text-gray-600 text-xl font-bold">x</button>
        </div>
          ...
          {showAddVehicle ? (
            <div className="bg-blue-50 rounded-xl p-4 mb-4 border border-blue-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[{k:"driver_name",l:"Driver Name"},{k:"vehicle_plate",l:"Vehicle Plate"},{k:"vehicle_type",l:"Vehicle Type (e.g. Lorry, Van)"}].map(({k,l}) => (
                  <div key={k} className={k==="vehicle_type"?"col-span-2":""}>
                    <label className="text-xs text-gray-500 block mb-0.5">{l}</label>
                    <input value={newVehicle[k]} onChange={e => setNewVehicle(p => ({...p,[k]:e.target.value}))}
                ...
                <div>
                  ...
                  <select value={newVehicle.status} onChange={e => setNewVehicle(p => ({...p,status:e.target.value}))}
                    className="w-full border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      {[{k:"driver_name",l:"Driver Name"},{k:"vehicle_plate",l:"Vehicle Plate"},{k:"vehicle_type",l:"Vehicle Type"}].map(({k,l}) => (
                        <div key={k}>
                          ...
                          <input value={editVehicle[k] || ""} onChange={e => setEditVehicle(p => ({...p,[k]:e.target.value}))}
                      ...
                      <div>
                        ...
                        <select value={editVehicle.status || "Active"} onChange={e => setEditVehicle(p => ({...p,status:e.target.value}))}
                          className="w-full border rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-300">
                          <option>Active</option><option>Inactive</option>
                        </select>
                      </div>
        ...
        <div className="px-6 py-4 border-t flex justify-end">
          <button onClick={() => { setShowVehicleModal(false); setShowAddVehicle(false); setEditVehicleId(null); }}
            className="px-5 py-2 text-sm bg-gray-100 rounded-lg hover:bg-gray-200">Close</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full mx-4">
        ...
        <div className="mb-3">
          ...
          <select value={newRoute.vehicle_id} onChange={e => onVehicleSelect(e.target.value)}
            className="w-full border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-300">
          ...
          {activeVehicles.length === 0 && (
            <p className="text-xs text-orange-500 mt-1">No active vehicles. <button onClick={() => { setShowAddRoute(false); setShowVehicleModal(true); }} className="underline">Add vehicle first</button></p>
          )}
        </div>
        ...
        <div className="flex gap-3 justify-end mt-4">
          <button onClick={() => { setShowAddRoute(false); setNewRoute({ ...EMPTY_ROUTE }); }} className="px-4 py-2 text-sm bg-gray-100 rounded-lg hover:bg-gray-200">Cancel</button>
    ...
    <div className="max-w-7xl mx-auto px-4 py-4">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setShowAddRoute(true); setNewRoute({ ...EMPTY_ROUTE }); }} className="bg-blue-600 text-white rounded-lg px-4 py-1.5 text-xs font-medium hover:bg-blue-700">+ Add Route</button>
        </div>
      </div>
