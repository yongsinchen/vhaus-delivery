...
// Override commissions (a person earning a % of a branch's sales, not their
      ...
      {tab === 0 && (
        <div className="space-y-4">
          ...
          {!payoutLoading && payout && visibleUsers.map(u => {
            // Split the person's OWN sales from override earnings (a % of other
            // people's branch sales) so the two are never conflated.
            const ownComms = u.commissions.filter(c => !isOverrideRow(c));
            const overrideComms = u.commissions.filter(isOverrideRow).sort(bySoAsc);
            ...
            <div key={u.user_id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  {overrideComms.length > 0 && <p className="text-xs font-semibold text-indigo-600 mt-0.5">Override: {money(overrideTotal)} on {overrideComms.length} order(s)</p>}
                </div>
