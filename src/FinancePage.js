        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setDetailTxn(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-4 space-y-3 text-sm">
              {detailTxn.or_number != null && <div className="flex justify-between"><span className="text-gray-500">Payment Acknowledgement No.</span><span className="font-bold text-gray-900">#{detailTxn.or_number}</span></div>}
            ...
            <div className="flex items-center justify-between gap-2 px-5 py-3 border-t border-gray-100">
              <span className="text-xs text-gray-400">{detailTxn.or_number != null ? `Payment Acknowledgement #${detailTxn.or_number}` : "No OR number"}</span>
              <button onClick={() => printReceipt(detailTxn)} className={`px-3 py-1.5 rounded-lg text-white text-sm font-medium ${apprOf(detailTxn) === "rejected" ? "bg-gray-500 hover:bg-gray-600" : "bg-violet-600 hover:bg-violet-700"}`}>🧾 {apprOf(detailTxn) === "rejected" ? "Print VOID copy" : "Print Payment Acknowledgement"}</button>
            </div>
          </div>
        </div>
      )}
